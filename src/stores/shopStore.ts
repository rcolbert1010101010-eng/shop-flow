...
import { moneyRound, moneySafe, normalizePhone, normalizeQty } from '@/lib/utils';
import type {
  SystemSettings,
  Customer,
  CustomerContact,
...
const applySalesOrderLinePricingDefaults = (line: SalesOrderLine): SalesOrderLine => ({
  ...line,
  unit_cost_snapshot: line.unit_cost_snapshot ?? null,
  suggested_unit_price: line.suggested_unit_price ?? null,
  pricing_rule_ids: line.pricing_rule_ids ?? [],
  pricing_scopes: line.pricing_scopes ?? [],
  pricing_flags: line.pricing_flags ?? [],
  pricing_explanation: line.pricing_explanation ?? null,
  price_overridden: line.price_overridden ?? false,
  price_override_reason: line.price_override_reason ?? null,
  price_overridden_by_user_id: line.price_overridden_by_user_id ?? null,
  price_override_approval_required: line.price_override_approval_required ?? false,
  price_approved_by_user_id: line.price_approved_by_user_id ?? null,
  priced_at: line.priced_at ?? line.created_at ?? now(),
});

const applyWorkOrderPartLinePricingDefaults = (line: WorkOrderPartLine): WorkOrderPartLine => ({
  ...line,
  unit_cost_snapshot: line.unit_cost_snapshot ?? null,
  suggested_unit_price: line.suggested_unit_price ?? null,
  pricing_rule_ids: line.pricing_rule_ids ?? [],
  ...
  persist<ShopState>(
      ...
      return {
        ...
        addPart: (part) => {
          ...
          const newPart: Part = {
            ...
            is_remnant: part.is_remnant ?? false,
            parent_part_id: part.parent_part_id ?? null,
            remnant_width_in: part.remnant_width_in ?? null,
            remnant_length_in: part.remnant_length_in ?? null,
        ...
        soAddPartLine: (orderId, partId, qty) => {
          ...
          const unitPrice = moneySafe(suggested ?? part.selling_price ?? 0);
          ...
          if (existingLine) {
            ...
            const lineTotal = moneyRound(newQty * moneySafe(existingLine.unit_price));
            ...
            const newLine: SalesOrderLine = {
              ...
              line_total: moneyRound(normalizedQty * unitPrice),
              ...
              price_override_approval_required: false,
        ...
        soUpdatePartQty: (lineId, newQty) => {
          ...
          const lineTotal = moneyRound(normalizedQty * moneySafe(line.unit_price));
        ...
        soUpdateLineUnitPrice: (lineId, newUnitPrice) => {
          ...
          const updatedLineTotal = moneyRound(moneySafe(line.quantity) * moneySafe(newUnitPrice));
        ...
        soMarkCoreReturned: (lineId) => {
          ...
          // Create refund line
          const refundUnitPrice = -moneySafe(line.core_charge);
          const refundLineTotal = moneyRound(refundUnitPrice * moneySafe(line.quantity));
        ...
        addSalesOrderChargeLine: (line) => {
          const qty = moneySafe(line.qty);
          if (qty <= 0) return null;
          const timestamp = now();
          const unit_price = moneySafe(line.unit_price);
          const chargeLine: SalesOrderChargeLine = {
            ...line,
            id: line.id ?? generateId(),
            qty,
            unit_price,
            total_price: moneyRound(qty * unit_price),
            created_at: timestamp,
            updated_at: timestamp,
          };
          set((state) => ({
        ...
        updateSalesOrderChargeLine: (id, patch) => {
          ...
          if (!existing) return;
          const qty = moneySafe(patch.qty ?? existing.qty);
          const unit_price = moneySafe(patch.unit_price ?? existing.unit_price);
        ...
        recalculateSalesOrderTotals: (orderId: string) => {
          ...
          const subtotal = moneyRound(lines.reduce((sum, l) => {
            if (l.is_warranty) return sum;
            // Refund lines have negative line_total and should be included
            return sum + moneySafe(l.line_total);
          }, 0));

          const charge_subtotal = moneyRound(
            chargeLines.reduce((sum, l) => sum + moneySafe(l.total_price), 0)
          );

          // Calculate core charges (only for non-returned cores, exclude refund lines which are already in subtotal)
          const core_charges_total = moneyRound(lines.reduce((sum, l) => {
            if (l.is_core_refund_line) return sum; // Refund lines already included in subtotal
            const coreCharge = moneySafe(l.core_charge);
            if (coreCharge > 0 && !l.core_returned) {
              return sum + coreCharge * moneySafe(l.quantity);
            }
            return sum;
          }, 0));
          ...
          const taxableAmount = moneyRound(subtotal + core_charges_total + charge_subtotal);
          const safeTaxRate = moneySafe(tax_rate);
          const tax_amount = moneyRound(taxableAmount * (safeTaxRate / 100));
          const total = moneyRound(taxableAmount + tax_amount);
        ...
        woUpdatePartQty: (lineId, newQty) => {
          ...
          const normalizedQty = qtyResult.qty;
          const qtyChange = normalizedQty - line.quantity;
          const lineTotal = moneyRound(normalizedQty * moneySafe(line.unit_price));
        ...
        woClockIn: (workOrderId, jobLineId, technicianId, technicianName) => {
          ...
          const closeMatchingEntry = (entry: WorkOrderTimeEntry) => {
            ...
            logWorkOrderActivity({
              ...
              type: 'CLOCK_OUT',
              message: `Clock out: ${finished.technician_name || 'Technician'} → ${finishedJob?.title ?? 'Job'
                }`,
            });
          };
        ...
        recalculateWorkOrderTotals: (orderId: string) => {
          ...
          const parts_subtotal = moneyRound(partLines.reduce((sum, l) => {
            if (l.is_warranty) return sum;
            return sum + moneySafe(l.line_total);
          }, 0));

          // Core charges (only for non-returned cores, exclude refund lines)
          const core_charges_total = moneyRound(partLines.reduce((sum, l) => {
          ...
          const labor_subtotal = moneyRound(
            laborLines.reduce((sum, l) => sum + (l.is_warranty ? 0 : moneySafe(l.line_total)), 0)
          );
          ...
          for (const entry of timeEntries) {
            ...
            if (technician) {
              const hours = moneySafe(entry.total_minutes) / 60;
              labor_cost += hours * moneySafe(technician.hourly_cost_rate);
            }
          }
          labor_cost = moneyRound(labor_cost);

          const subtotal = moneyRound(parts_subtotal + labor_subtotal + charge_subtotal + core_charges_total);
          ...
          const safeTaxRate = moneySafe(tax_rate);
          const tax_amount = moneyRound(subtotal * (safeTaxRate / 100));
          const total = moneyRound(subtotal + tax_amount);
    ...
    },
    {
      ...
      merge: (persistedState, currentState) => {
        ...
        const salesOrderLines = state.salesOrderLines ?? currentState.salesOrderLines;
        const workOrderPartLines = state.workOrderPartLines ?? currentState.workOrderPartLines;
        return {
          ...currentState,
          ...state,
          salesOrderLines: salesOrderLines.map(applySalesOrderLinePricingDefaults),
          workOrderPartLines: workOrderPartLines.map(applyWorkOrderPartLinePricingDefaults),
        };
      },
    }
  )
