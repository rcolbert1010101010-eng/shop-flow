...
create table if not exists public.accounting_integration_config (
  ...
  mode text not null default 'INVOICE_ONLY',
  calculation_source text not null default 'SHOPFLOW',
  ...
  line_item_strategy text not null default 'ROLLUP',
  customer_match_strategy text not null default 'DISPLAY_NAME',
...
-- RLS policies (uses current_app_role helper)
